import { Program, makeProgram as makeL3Program, makeProgram, parseL3 } from './L3/L3-ast';
...
import {
    ...
    CExp, Exp, AppExp, DictExp, DictEntry, VarDecl, VarRef,
...
export const EntriesToLitExp = (entries: DictEntry[]): LitExp => 
    makeLitExp(reduce(
        (acc: SExpValue, next: CompoundSExp) =>
            makeCompoundSExp(
                next,
                acc
            ),
...
export const CExpToSExp = (exp: CExp|VarDecl|Binding): SExpValue => 
    ...
    isProcExp(exp) ? makeCompoundSExp(makeSymbolSExp("lambda"), makeCompoundSExp(
        reduce((acc: SExpValue, next: SExpValue)=>makeCompoundSExp(next, acc),makeEmptySExp(),map(CExpToSExp, exp.args)), 
        reduce((acc: SExpValue, next: SExpValue)=>makeCompoundSExp(next, acc),makeEmptySExp(),map(CExpToSExp, exp.body)))) :
    ...
    isAppExp(exp) ? makeCompoundSExp(CExpToSExp(exp.rator), reduce((acc: SExpValue, next: SExpValue)=>makeCompoundSExp(next, acc),makeEmptySExp(),map(CExpToSExp, exp.rands))):
    isLetExp(exp) ? makeCompoundSExp(makeSymbolSExp("let"), makeCompoundSExp(
        reduce((acc: SExpValue, next: SExpValue)=>makeCompoundSExp(next, acc),makeEmptySExp(), map((x:Binding)=>CExpToSExp(x) ,exp.bindings)),
        reduce((acc: SExpValue, next: SExpValue)=>makeCompoundSExp(next, acc),makeEmptySExp(), map(CExpToSExp, exp.body)))) :
